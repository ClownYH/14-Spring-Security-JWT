    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if(header != null && !header.equalsIgnoreCase("")){
                ...
                if(TokenUtils.isValidToken(token)){ // 토큰이 유효할 경우
                    Claims claims = TokenUtils.getClaimsFormToken(token); // 토큰 복호화
                    ...
                    SecurityContextHolder.getContext().setAuthentication(authenticationToken); // 권한이 필요한 요청 리소스가 들어오면 확인해주는 곳에 토큰 담기
                    chain.doFilter(request, response);
                }else {
